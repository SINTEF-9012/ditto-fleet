...
exports.StandardResilienceHandlerFactory = exports.StandardResilienceHandler = void 0;
const websocket_resilience_buffer_1 = require("./websocket-resilience-buffer");
 ...
 */
class StandardResilienceHandler extends websocket_resilience_interfaces_1.AbstractResilienceHandler {
    constructor(webSocketBuilder, stateHandler, requestHandler, size) {
        ...
        if (size !== undefined) {
            if (size > 0) {
                this.requestBuffer = new websocket_resilience_buffer_1.ResilienceRequestBuffer(size);
                this.messageBuffer = new websocket_resilience_buffer_1.ResilienceMessageBuffer(size);
            }
            else {
            ...
            this.requestBuffer = new websocket_resilience_buffer_1.ResilienceRequestBuffer();
            this.messageBuffer = new websocket_resilience_buffer_1.ResilienceMessageBuffer();
        }
    }
    sendRequest(correlationId, request) {
        ...
        if (this.stateHandler.isBuffering()) {
            if (!this.stateHandler.isWorking()) {
                this.rejectRequest(correlationId, websocket_resilience_interfaces_1.connectionLostError);
            }
            else {
    ...
    send(message) {
        if (!this.stateHandler.canSend()) {
            if (!this.stateHandler.isWorking()) {
                return Promise.reject(websocket_resilience_interfaces_1.connectionLostError);
            }
            if (this.messageBuffer.full()) {
                this.stateHandler.bufferFull();
                return Promise.reject(websocket_resilience_interfaces_1.bufferFullError);
            }
...
exports.StandardResilienceHandler = StandardResilienceHandler;
/**
 * A Factory for a StandardResilienceHandler.
 */
class StandardResilienceHandlerFactory extends websocket_resilience_interfaces_1.ResilienceHandlerFactory {
    constructor(size) {
        super();
        this.size = size;
    }
...
exports.StandardResilienceHandlerFactory = StandardResilienceHandlerFactory;
