...
const DEFAULT_LOG_FILE = "../logs/timestamps.csv";
 ...
 */
function logTimestamp({ workflowId, step, event, nodeId = os.hostname(), file = DEFAULT_LOG_FILE }) {
 ...
 * @param {string} [opts.nodeId] - Device / container hostname
 */
function logTimestampJS({ workflowId, step, event, nodeId = os.hostname() }) {
  const timestamp = Date.now();
  const timestampISO = new Date().toISOString()
  const line = `${workflowId},${step},${event},${timestamp},${nodeId}${os.EOL}`;
  //const filePath = path.resolve(file);
